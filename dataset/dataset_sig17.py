...
class Img_Dataset(Dataset):
    def __init__(self, ldr_path, label_path, exposure_path, patch_size):
        ...
        self.expo_times = []
        with open(exposure_path) as lines:
            for line in lines:
                self.expo_times.append(int(line[0]))
    ...
    def __getitem__(self, index):
        ...
        pre_img0 = gamma_correction(self.ldr_patches[index][0], self.expo_times[0], 2.2)
        pre_img1 = gamma_correction(self.ldr_patches[index][1], self.expo_times[1], 2.2)
        pre_img2 = gamma_correction(self.ldr_patches[index][2], self.expo_times[2], 2.2)
        pre_img0 = np.concatenate((self.ldr_patches[index][0], pre_img0), 2)
        pre_img1 = np.concatenate((self.ldr_patches[index][1], pre_img1), 2)
        pre_img2 = np.concatenate((self.ldr_patches[index][2], pre_img2), 2)
        img0 = pre_img0.astype(np.float32).transpose(2, 0, 1)
        img1 = pre_img1.astype(np.float32).transpose(2, 0, 1)
        img2 = pre_img2.astype(np.float32).transpose(2, 0, 1)
        img0 = torch.from_numpy(img0)
